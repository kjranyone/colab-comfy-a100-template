...
### 🛑 【最重要・確実度100%】課金（CU消費）を確実に止める手順
検証や生成が終わったら、セルを実行しようとせず **Colab メニューから直接切断** してください：

1. Colab 画面上部メニューの **「ランタイム」** をクリック
2. **「ランタイムの接続を解除して削除」** を選択（確認で「はい」）

> 💡 **なぜこれが一番確実なのか？**:  
> セルが実行中（Step 4稼働中）であったり、Python カーネルがフリーズしていても、Google Cloud の仮想マシン管理基盤（ハイパーバイザ）が直接 A100 インスタンスを破棄するため、**1秒のタイムラグもなく 100% 確実に課金メーターがストップ** します。成果物（動画）は Google Drive（`MyDrive/ComfyUI_Outputs/`）にリアルタイム保存されているため、切断しても安全です。
